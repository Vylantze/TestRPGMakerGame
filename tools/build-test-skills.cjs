// TestSkills.json is authoritative. Export an editor-readable MZ skill database.
const fs = require("node:fs");
const path = require("node:path");
const root = path.resolve(__dirname, "..");
const skills = require(path.join(root, "src/data/TestSkills.json"));
...
for (const [key, skill] of Object.entries(skills)) {
    ...
    rows[id] = {
        ...
        note: "<TestSkill:" + key + ">\n<SourceSkill:" + source.id + ">\nOverworld execution uses TestSkills.json. Editor TP cost represents SP; no engine battle scene is used."
    };
}
...
console.log("Exported " + Object.keys(skills).length + " TestSkills entries to Skills.json; original archive unchanged.");
